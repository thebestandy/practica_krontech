...
class RiseProjectScraper:
    ...
    def fetch_json(self, url: str, params: dict | None = None) -> object | None:
    ...
    def query_matches(self, text: str, query: str) -> bool:
    ...
    def extract_title_from_api(self, item: dict) -> str | None:
        title = item.get("title")

        if isinstance(title, dict):
            return self.clean_text(title.get("rendered"))
    ...
    def extract_text_from_api(self, item: dict) -> str:
        parts = []
        ...
        for field in ["content", "excerpt"]:
            ...
            if isinstance(value, dict) and value.get("rendered"):
                parts.append(self.clean_html_content(value["rendered"]))
            elif isinstance(value, str):
                parts.append(self.clean_html_content(value))

        return self.clean_text(" ".join(parts))

    def extract_article_links_from_api(self, query: str) -> list[dict]:
        articles = []
        seen_urls = set()
        ...
        if isinstance(search_data, list):
            for item in search_data:
                ...
                if not url or not self.is_valid_article_url(url) or url in seen_urls:
                    continue
        ...
        for endpoint in WP_ENDPOINTS:
            ...
            for item in data:
                ...
                seen_urls.add(url)
                articles.append({
                    "title": self.extract_title_from_api(item),
                    "url": url,
                    "api_item": item,
                })
    ...
    def extract_article_links_from_html(self, query: str) -> list[dict]:
        ...
        for link in soup.find_all("a", href = True):
            ...
            if not self.is_valid_article_url(url) or url in seen_urls:
                continue
    ...
    def extract_article_links(self, query: str) -> list[dict]:
        articles = self.extract_article_links_from_api(query)

        if articles:
            return articles

        return self.extract_article_links_from_html(query)

    def extract_article_details_from_api(self, item: dict) -> dict:
        return {
            "title": self.extract_title_from_api(item),
            "date": self.normalize_date(item.get("date") or item.get("date_gmt")),
            "text": self.extract_text_from_api(item),
            "is_sponsored": False,
        }

    def extract_article_details_from_html(self, url: str) -> dict:
        ...
        return {
            ...
            "text": self.clean_html_content(str(article_container)),
    ...
    def extract_article_details(self, article: dict) -> dict:
        ...
        if article.get("api_item"):
            details = self.extract_article_details_from_api(article["api_item"])

        if not details or not details.get("text"):
            details = self.extract_article_details_from_html(article["url"])

        return details

    def search(self, target: str, limit: int = 5) -> dict:
        article_links = self.extract_article_links(target)
        graph_nodes = []
        seen_urls = set()

        with ThreadPoolExecutor(max_workers = min(MAX_WORKERS, max(1, len(article_links)))) as executor:
            for article, details in zip(
                article_links,
                executor.map(self.extract_article_details, article_links),
            ):
                url = article["url"]
                ...
                if not self.query_matches(searchable_text, target):
                    continue
                ...
                graph_nodes.append({
                    ...
                    "type": "Document",
                    "label": details.get("title") or article.get("title"),
                    "summary": details.get("date") or "Data necunoscuta",
                    "url": url,
