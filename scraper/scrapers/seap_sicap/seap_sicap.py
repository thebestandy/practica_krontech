...
class ElicitatieScraper:
    ...
    def export_to_csv(self, contracts: List[Contract], filename: str):
        ...
        with open(filename, 'w', newline = '', encoding = 'utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['Contract ID', 'Title', 'Contracting Authority', 'Supplier',
                             'Value', 'Currency', 'Date', 'Procedure Type',
                             'CPV Code', 'Status', 'Description', 'Source URL'])
            for c in contracts:
                writer.writerow([c.contract_id, c.title, c.contracting_authority,
                                 c.supplier, c.contract_value, c.currency,
                                 c.contract_date, c.procedure_type, c.cpv_code,
                                 c.status, c.description, c.source_url])
        print(f"Exported {len(contracts)} contracts to {filename}")

    def export_to_json(self, contracts: List[Contract], filename: str):
        with open(filename, 'w', encoding = 'utf-8') as f:
            json.dump([asdict(c) for c in contracts], f, indent = 2, ensure_ascii=False)
        print(f"Exported {len(contracts)} contracts to {filename}")
